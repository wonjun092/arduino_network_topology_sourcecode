...
byte localSeq = 0;
byte seenSequence[6] = {0, 0, 0, 0, 0, 0};
...
void setup() {
  ...
  Serial.println("=== MESH Topology System ===");
}
  ...
  if (isNodeDead) {
    ...
    if (Serial.available() > 0) {
      ...
      if (cmd == 'g' || cmd == 'G') {
        ...
        sentPacketCount = 0;
        localSeq = 0;
        // Reset duplicate filtering history for each new experiment.
        for (int i = 0; i <= 5; i++) {
          seenSequence[i] = 0;
        }
        lastSendTime = time_now;
        Serial.println("# [START] MESH Experiment Started.");
      }
    }
    ...
    if (isExperimentStarted && (sentPacketCount < MAX_PACKET_COUNT)) {
      if (time_now - lastSendTime >= SEND_INTERVAL) {
        ...
        seenSequence[MY_NODE_ID] = localSeq;
        floodPacket(MY_NODE_ID, TARGET_NODE_ID, localSeq);
...
void handleCANReceive() {
  ...
  if (CAN_MSGAVAIL == CAN.checkReceive()) {
    ...
    if (rxId == CAN_ID_BASE) {
      ...
      if (seenSequence[origin] >= seq) {
        return;
      }
      seenSequence[origin] = seq;
      ...
      if (target == MY_NODE_ID) {
        ...
        delay(2);
        floodPacket(origin, target, seq);
      }
    }
  }
}

void floodPacket(byte origin, byte target, byte seq) {
  unsigned char buf[8] = {origin, target, seq, 0, 0, 0, 0, 0};
  CAN.sendMsgBuf(CAN_ID_BASE, 0, 8, buf);
}
