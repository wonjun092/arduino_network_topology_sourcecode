...
// [실험 설정 변수]
const byte MY_NODE_ID = 2;       // 중앙 허브 노드는 1로 설정, 나머지는 2~5
...
void setup() {
  ...
  Serial.println("=== STAR Topology System ===");
}
  ...
  if (isNodeDead) {
    ...
    if (Serial.available() > 0) {
      ...
      if (cmd == 'g' || cmd == 'G') {
        ...
        Serial.println("# [START] STAR Experiment Started.");
      }
    }
    ...
    if (isExperimentStarted && (sentPacketCount < MAX_PACKET_COUNT)) {
      if (time_now - lastSendTime >= SEND_INTERVAL) {
        ...
        // STAR 구조: 내가 허브(1)가 아니라면 무조건 1번 허브 노드로 전송
        if (MY_NODE_ID == 1) {
          sendPacket(TARGET_NODE_ID, MY_NODE_ID, TARGET_NODE_ID, localSeq);
        } else {
          sendPacket(1, MY_NODE_ID, TARGET_NODE_ID, localSeq); 
        }
...
void handleCANReceive() {
  ...
  if (CAN_MSGAVAIL == CAN.checkReceive()) {
    ...
    if (logicalNextHop == MY_NODE_ID) {
      ...
      // 중앙 허브 노드(1)인 경우 중계(Forwarding) 수행
        ...
        delay(2);
        sendPacket(target, origin, target, seq);
      }
    }
  }
}
