...
void setup() {
  ...
  // 링 라우팅 설정 (1->2->3->4->5->1)
  myNextHop = (MY_NODE_ID % 5) + 1; 
  
  Serial.println("=== RING Topology System ===");
}
  ...
  if (isNodeDead) {
    ...
    if (Serial.available() > 0) {
      ...
      if (cmd == 'g' || cmd == 'G') {
        ...
        Serial.println("# [START] RING Experiment Started.");
      }
    }
    ...
    if (isExperimentStarted && (sentPacketCount < MAX_PACKET_COUNT)) {
      if (time_now - lastSendTime >= SEND_INTERVAL) {
        ...
        // RING 구조: 내 다음 순서 노드에게 무조건 전달
        sendPacket(myNextHop, MY_NODE_ID, TARGET_NODE_ID, localSeq);
  ...
  if (CAN_MSGAVAIL == CAN.checkReceive()) {
    ...
    if (logicalNextHop == MY_NODE_ID) {
      ...
      if (target == MY_NODE_ID) {
        ...
        delay(2);
        sendPacket(myNextHop, origin, target, seq);
      }
    }
  }
}
